...
def run_graph(graph_path: str, source: int, sink: int, family: str = "", seed: int = 0, trial: int = 0, logger: logging.Logger = None):
    ...
    dinic = Dinics(graph_path=graph_path, source=source, sink=sink, logger=logger)
...
def main():
    ...
    # Sample graphs configuration: (filename, source, sink, family)
    samples = [
    ...
    for filename, source, sink, family in samples:
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"  ✓ Completed: max_flow={max_flow}, runtime={runtime:.4f}s")
            else:
    ...
    for filename, source, sink, family in families:
        graph_path = os.path.join(graphs_dir, filename)
        logger.info(f"\nProcessing: {filename} (source={source}, sink={sink}, family={family})")
        try:
            result = run_graph(graph_path, source, sink, family=family, logger=logger)
            if result:
                max_flow, runtime = result
