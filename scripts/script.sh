...
for g in "${GRAPH_FOLDERS[@]}"; do
  ...
  for tgt_base in "${TARGETS[@]}"; do
    # Copy graphs into the target base under the same folder name. We do not
    # create mode-specific input folders anymore — the batch runner will
    # execute both traversal methods (classical/edmondson) per input graph.
    tgt_dir="$tgt_base/$g"
